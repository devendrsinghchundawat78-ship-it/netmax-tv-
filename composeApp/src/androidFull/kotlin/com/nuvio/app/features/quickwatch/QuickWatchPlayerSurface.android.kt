...
actual fun QuickWatchPlayerSurface(
    ...
    val latestOnProgress = rememberUpdatedState(onProgress)
    var playerContainer by remember { mutableStateOf<QuickWatchTextureContainer?>(null) }
    ...
    LaunchedEffect(exoPlayer, playWhenReady) {
        while (isActive && playWhenReady) {
            val dur = exoPlayer.duration.coerceAtLeast(0L)
            val pos = exoPlayer.currentPosition.coerceAtLeast(0L)
            if (dur > 0L) {
                latestOnProgress.value(pos, dur)
            }
            delay(250)
        }
    }
    ...
    AndroidView(
        modifier = modifier,
        factory = { viewContext ->
            QuickWatchTextureContainer(viewContext).apply {
...
private class QuickWatchTextureContainer(
    context: Context,
) : FrameLayout(context) {
    ...
    private fun updateTextureTransform() {
        ...
        // Center-crop video to fill the vertical screen completely
